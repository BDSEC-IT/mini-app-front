  ...
  return (
    <div className="col-span-3 bg-gray-50 dark:bg-gray-800/30 rounded-lg p-1.5 border border-gray-200 dark:border-gray-700">
      ...
      <div className="grid grid-cols-4  text-[10px] font-medium text-gray-600 dark:text-gray-300 pb-1">
                <div className="text-center">{t('dashboard.buyQuantity', 'Авах тоо')}</div>
        <div className="text-green-600 text-center">{t('exchange.buy', 'Авах үнэ')}</div>

        <div className="text-red-600 text-center">{t('exchange.sell', 'Зарах үнэ')}</div>
        <div className="text-center">{t('dashboard.sellQuantity', 'Зарах тоо')}</div>
      </div>
          ...
          Array.from({ length: Math.max(orderBook.sell?.length || 0, orderBook.buy?.length || 0) }).map((_, index) => {
            ...
            return (
              <div key={index} className="grid pr-5 grid-cols-4 gap-x-4 text-[10px]">
                {/* Buy side - clickable row (when clicked, user should SELL to match this buy order) */}
                <button 
                  onClick={() => {
                    if (bidOrder) {
                      onOrderClick('SELL', bidOrder.price.toString(), bidOrder.quantity.toString());
                    }
                  }}
                ...
                {/* Sell side - clickable row (when clicked, user should BUY to match this sell order) */}
                <button 
                  onClick={() => {
                    if (askOrder) {
                      onOrderClick('BUY', askOrder.price.toString(), askOrder.quantity.toString());
                    }
                  }}
