...
export default function CalculateInterest({
  ...
  const [selectedBonds, setSelectedBonds] = useState<CalculateBond[]>(bonds);
  ...
  const handleBondSelect = (bondId: number) => {
    const bond = selectedBonds.find((bond) => bond.pkId === bondId);
  ...
  const handleExchangeSelect = (id: string) => {
    //1. mse
    //2. otc
    const isMse = id === '1';
    const isOtc = id === '2';
    const bond = isMse ? bonds[0] : isOtc ? otcBonds![0] : bonds[0];
    const bondsSelection = isMse ? bonds : isOtc ? otcBonds! : bonds;
    setSelectedExchange(id);
    console.log(bond);
    setSelectedBond(bond || null);
    setSelectedBonds(bondsSelection);
    setSellBoughtPrice(bond?.NominalValue ? bond.NominalValue : '');
    setDays('');
    setSellDays('');
    // setPrice('');
  ...
  function getRemainingDays(refundDate: string): number {
    // Standardize format (remove time if present)
    const dateStr = refundDate.split(' ')[0];
    const refund = new Date(dateStr);
    const today = new Date();

    // Calculate difference in milliseconds and convert to days
    const diffMs = refund.getTime() - today.getTime();
  ...
  return (
    <Tabs defaultValue="buy" className="">
      ...
      <TabsContent value="buy">
        <div
          className=".flex-col flex min-h-[45dvh]  flex-col justify-between  "
          tabIndex={0}
        >
          {selectedBond && (
            <div className="text-center ">
              <div className="mx-3 mb-5 flex h-5 items-center justify-center space-x-4 py-8 text-xs sm:my-5  lg:py-2">
                <div className="">
          ...
          <div className=".md:grid-cols-2 grid grid-cols-1 gap-5">
            <div className="space-y-3">
              <div className="grid grid-cols-4 items-center gap-4">
                ...
                <Select
                  onValueChange={(value) => handleExchangeSelect(value)}
                  value={selectedExchange || '1'}
                  defaultValue="1"
                >
                  <SelectTrigger className="col-span-3">
                    <SelectValue placeholder={t('birjselect')} />
                  </SelectTrigger>
                  <SelectContent>
                    <ScrollArea className="max-h-48 overflow-y-auto md:max-h-64">
                      {/* Render the Mongolian Stock Exchange with value as "1" */}
                      <SelectItem value="1">{t('mse')}</SelectItem>

                      {/* Conditionally render OTC with value as "2" if otcBonds exists */}
                      {otcBonds && otcBonds.length > 0 && (
                        <SelectItem value="2">{t('otc')}</SelectItem>
                      )}
                    </ScrollArea>
                  </SelectContent>
                </Select>
              ...
              <div className="grid grid-cols-4 items-center gap-4">
                ...
                <Select
                  ...
                  defaultValue={selectedBonds[0]?.pkId.toString() || ``}
                  value={selectedBond?.pkId.toString() || ''}
                >
                  <SelectTrigger className="col-span-3">
                  ...
                  <SelectContent>
                    <ScrollArea className="max-h-48 overflow-y-auto md:max-h-64">
                      {selectedBonds.map((bond) => (
                        <SelectItem
                          key={bond.pkId}
                          placeholder={t('bondselect')}
                          value={bond.pkId.toString()}
                        >
                          {bond.BondenName}{' '}
                          <span className="text-xs italic text-muted-foreground">
                            ({bond.Interest}) (
                            {getRemainingDays(bond.RefundDate)} {t('day')})
                          </span>
                        </SelectItem>
                      ))}
                    </ScrollArea>
              ...
              <p className="my-0 text-right text-xs italic leading-none text-muted-foreground">
                {t('end1')}{' '}
                {selectedBond ? getRemainingDays(selectedBond.RefundDate) : '-'}{' '}
                {t('end2')}
              </p>
              {selectedBond && (
                <>
              ...
              <div className="flex-1 space-y-1">
                  ...
                  (selectedOption === 'default' ||
                    ...
                    <>
                      ...
                      <span className="ml-1 inline-flex text-xs italic  text-green-400">
                        {' '}
                        ({((Number(calculateYield()) / price) * 100).toFixed(2)}
                        %)
                ...
                <p className="text-xs text-muted-foreground">
                  ...
                  {selectedOption === 'default' && selectedBond ? (
                    <>
                      {format(new Date(selectedDate), 'yyyy-MM-dd')} -{' '}
                      {format(new Date(selectedBond.RefundDate), 'yyyy-MM-dd')}{' '}
                      (
                      {Math.ceil(
                        (new Date(selectedBond.RefundDate).getTime() -
      ...
      <TabsContent value="sell">
        ...
        >
          {selectedBond && (
            <div className="text-center ">
              <div className="mx-3 my-5 mb-5 flex h-5 items-center justify-center space-x-4 text-xs sm:py-5 lg:py-2">
                <div className="">
                  {t('yearlyInterest')}: {selectedBond.Interest}
                </div>
                <Separator orientation="vertical" />
